        ...
        if (data.fatal) {
          switch (data.type) {
            case Hls.ErrorTypes.NETWORK_ERROR:
              // Si la carga directa falló por CORS, token, contenido mixto o bloqueo de red,
              // reintentar automáticamente a través del proxy (/api/stream-proxy)
              if (
                !streamUrl.includes('/api/stream-proxy') &&
                channel?.url
              ) {
                console.log('[Player] Fallo directo de red/CORS. Reintentando con proxy:', channel.url);
                hls.destroy();
